...
uniform float slices; // = 50.0;
uniform float startAmplitud; // = 0.01;
uniform float startFrequency; // = 1.25;
uniform float startDensity; // = 00.0;
uniform float animationSpeed; // = 0.0075;
...
void main(void){
	...
	if (intersectSphere(o, d, tmin, tmax)){	
		...
		for (float i = 0.0; i < slices; i+=1.0){
			...
			// get turbulence in d
			float a = startAmplitud;
			float b = startFrequency;
			float d = startDensity;
			for (int j = 0; j < 3; j++)									
			{
				d += 0.5/abs((fbm(5.0*curpos*b+animationSpeed*time/b)*2.0-1.0)/a);
				b *= 2.0;
				a /= 2.0;
			}
